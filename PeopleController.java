...
public class PeopleController {
  ...
  public ResponseEntity<Map<String, List<Person> >> listUserInEachCity() {
    return ResponseEntity.ok().body(personRepo.listUserInEachCity());
  }

  // 2.
  @GetMapping("youngestdevs")
  public ResponseEntity<Map.Entry<String, Double>> averageAgeCity() {
    return ResponseEntity.ok().body(personRepo.averageAgeCity());
  }

  // 3.
  @GetMapping("malefemaleratio")
  public ResponseEntity<Double> maleFemaleRatioInCity() {
    return ResponseEntity.ok().body(personRepo.maleFemaleRatioInCity());
  }

  // 4.
  @GetMapping("avgsalarypeopleabove30")
  public ResponseEntity <Double> averageSalary() {
    return ResponseEntity.ok().body(personRepo.averageSalary());
  }

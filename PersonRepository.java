...
public class PersonRepository {
    ...
    */
    ...
    //HomeWork
    //1. Trong mỗi thành phố Hanoi, Saigon, Shanghai, hãy  Trả về danh sách 'developer'
    public Map<String, List<Person>> listUserInEachCity() {
        return people
                .stream()
                .filter(p -> p.getJob().equals("developer"))
                .filter(p -> ("Hanoi, Saigon, Shanghai").contains(p.getCity()))
                .collect(Collectors.groupingBy(Person::getCity));
    }

    // 2.Thành phố có độ tuổi trung bình của lập trình viên trẻ nhất
    public Map.Entry<String, Double> averageAgeCity() {
        Map<String, List<Person>> groupDevelopersByCity = people
                .stream()
                .filter(p -> p.getJob().equals("developer"))
                .collect(Collectors.groupingBy(Person::getCity));
        ...
        Optional<Map.Entry<String, Double>> result = cityAverageAge.entrySet()
                .stream()
    ...
    public  Double maleFemaleRatioInCity() {
        Map<String, List<Person>> groupDevelopersByCity = people
                .stream()
                .collect(Collectors.groupingBy(Person::getCity));

        Long maleCount = groupDevelopersByCity.entrySet()
                .stream()
                .map(p -> p.getValue()
                            .stream()
                            .filter(it -> it.getGender().equals("Male")).count())
                .collect(Collectors.counting());

        System.out.println(maleCount);

        return (double) maleCount / (double) (groupDevelopersByCity.size() - maleCount);
    }
